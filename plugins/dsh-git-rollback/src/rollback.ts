...
import { MAX_ROLLS, type RollbackRecord } from "./types.js";
 ...
 */
async function restoreTreeContent(gitBin: string, cwd: string, commit: string): Promise<{ ok: boolean; reason?: string }> {
  ...
  if (!clean.ok || !reset.ok) return { ok: false, reason: `clean/reset: ${clean.stderr || reset.stderr || ""}`.trim() };
  return { ok: true };
}
...
export async function performRedo(gitBin: string, cwd: string, sid: string, opts: RollbackOptions): Promise<CommandText> {
  const record = readRecord(cwd, sid);
  const roll = record && record.rolls.length > 0 ? record.rolls[record.rolls.length - 1] : undefined;
  if (!roll) return { kind: "error", text: "没有可重做的回退(先执行 /rollback)" };
  const verify = await gitExec(gitBin, cwd, ["rev-parse", "--verify", `${roll.redo}^{commit}`]);
  if (!verify.ok) return { kind: "error", text: "保存点已不可达(可能被 git gc),无法重做" };
  const currentUntracked = await untrackedList(gitBin, cwd);
  const restored = await restoreTreeContent(gitBin, cwd, roll.redo);
  if (!restored.ok) return { kind: "error", text: `重做失败:${restored.reason}` };
 ...
 */
export async function performUndo(gitBin: string, cwd: string, sid: string, rawInput: string, opts: RollbackOptions): Promise<CommandText> {
  const top = await gitExec(gitBin, cwd, ["rev-parse", "--show-toplevel"]);
  if (!top.ok || !top.stdout) return { kind: "error", text: "工作区不是 git 仓库,无法撤销" };
  const record = readRecord(cwd, sid);
  if (!record || record.checkpoints.length === 0) {
    return { kind: "error", text: "本会话还没有检查点(每个回合开始前自动快照)" };
  }
  ...
  if (raw === "") {
    ...
    if (!last) return { kind: "error", text: "没有可撤销的回合(需要该回合结束后的快照;旧记录的回合无结束快照,可用 /rollback N 整体回退)" };
    turn = last.turn;
  } else {
    if (!/^\d+$/.test(raw)) return { kind: "error", text: `无效的回合号:「${raw}」` };
  ...
  if (!diff.ok || !stats.ok) return { kind: "error", text: `生成回合 ${turn} 的改动失败:${diff.stderr || "git diff failed"}` };
  if (!diff.stdout.trim()) return { kind: "success", text: `回合 ${turn} 没有可撤销的文件改动。` };
...
export async function listCheckpoints(gitBin: string, cwd: string, sid: string, opts: RollbackOptions): Promise<CommandText> {
  const top = await gitExec(gitBin, cwd, ["rev-parse", "--show-toplevel"]);
  if (!top.ok || !top.stdout) return { kind: "error", text: "工作区不是 git 仓库" };
  const record = readRecord(cwd, sid);
  if (!record || record.checkpoints.length === 0) {
