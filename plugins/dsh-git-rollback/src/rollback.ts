...
export async function performRollback(
  ...
  if (!reset.ok) return { kind: "error", text: `回退失败:${reset.stderr || "reset --hard failed"}` };
...
export async function performRedo(gitBin: string, cwd: string, sid: string, opts: RollbackOptions): Promise<CommandText> {
  ...
  if (!reset.ok) return { kind: "error", text: `重做失败:${reset.stderr || "reset --hard failed"}` };
