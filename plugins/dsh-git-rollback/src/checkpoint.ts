...
export async function snapshotCommit(
  ...
  message: string,
): Promise<{ ok: boolean; commit?: string; tree?: string; unchanged?: boolean; reason?: string }> {
  ...
  if (idx.ok) {
    ...
    if (unmerged.ok && unmerged.stdout) return { ok: false, reason: "index has unmerged entries; skipping snapshot" };
    // 空索引:以空树还原
  }
  try {
    ...
    if (parent) {
      ...
      if (pt.ok) parentTree = pt.stdout;
    }
    if (parentTree !== undefined && tree.stdout === parentTree) return { ok: true, unchanged: true, tree: tree.stdout };
    const commit = await commitTree(gitBin, cwd, tree.stdout, parent, message);
    if (!commit.ok) return { ok: false, reason: `commit-tree: ${commit.stderr || "failed"}` };
...
async function currentTip(
  ...
  sid: string,
  refPrefix: string,
  record: RollbackRecord | undefined,
): Promise<{ commit?: string; fromRef: boolean }> {
  const tip = await gitExec(gitBin, cwd, ["rev-parse", "--verify", checkpointRef(refPrefix, sid)]);
  if (tip.ok && /^[0-9a-f]{40}$/.test(tip.stdout)) return { commit: tip.stdout, fromRef: true };
  const last = record?.checkpoints[record.checkpoints.length - 1];
  if (last && /^[0-9a-f]{40}$/.test(last.commit)) return { commit: last.commit, fromRef: false };
...
export async function checkpointTurn(
  ...
  if (!top.ok || !top.stdout) return; // 非 git 仓库:不检查点
  const record = readRecord(cwd, sid) ?? { version: 2, sessionId: sid, cwd, checkpoints: [], rolls: [] };
  if (record.checkpoints.some((c) => c.turn === turn)) return; // 幂等
  ...
  if (!parent) {
    ...
    if (head.ok) parent = head.stdout; // 首检查点父 = 当时 HEAD;unborn 时保持 undefined(根提交)
  }
  ...
  if (record.checkpoints.length > MAX_CHECKPOINTS) record.checkpoints = record.checkpoints.slice(-MAX_CHECKPOINTS);
 ...
 */
export async function checkpointTurnEnd(
  gitBin: string,
  cwd: string,
  sid: string,
  turn: number,
  ...
  if (!entry || entry.after) return; // 无开始检查点或已记录
  const snap = await snapshotCommit(gitBin, cwd, entry.commit, `${opts.commitPrefix}-end ${sid} turn ${turn}`);
  if (!snap.ok || snap.unchanged) {
    if (!snap.ok) console.error("[dsh-git-rollback] turn-end checkpoint failed:", snap.reason);
 ...
 */
export async function foldCheckpoints(gitBin: string, cwd: string, sid: string, opts: CheckpointOptions): Promise<CheckpointEntry[]> {
  const tip = await gitExec(gitBin, cwd, ["rev-parse", "--verify", checkpointRef(opts.refPrefix, sid)]);
  if (!tip.ok) return [];
  const out: CheckpointEntry[] = [];
  const seen = new Set<string>();
  let cursor: string | undefined = tip.stdout;
  while (cursor && /^[0-9a-f]{40}$/.test(cursor) && !seen.has(cursor)) {
    seen.add(cursor);
    const [meta, parentRes]: [Awaited<ReturnType<typeof gitExec>>, Awaited<ReturnType<typeof gitExec>>] = await Promise.all([
    ...
    if (!match) break; // 走到用户提交(首检查点的父)即停
    out.push({
